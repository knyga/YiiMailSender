 ...
 *
 * @author Anton Lyubch <anton.lyubch@gmail.com>, Oleksandr Knyga <oleksandrknyga@gmail.com>
 */
class MailSender extends CApplicationComponent
{
	...
	private function sender($to, $subject, $body)
	{
		...
		$message->setTo($to);
		//$message->setFrom(Yii::app()>params['email']['noreply'], Yii::app()>name);
	...
	{
		if(($pathOfAlias=Yii::getPathOfAlias($path))!==false
			  && file_exists($pathOfAlias)
		) {
	...
	public function send($email=array())
	{
		...
		if(is_array($email))
		{
			foreach($email as $value) {
				$this->sender($value,$subject,$message);
			}
		}
		else {
			$this->sender($email,$subject,$message);
		}
